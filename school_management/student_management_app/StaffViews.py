...
from .models import CustomUser, Staffs, Courses, Subjects, Students, SemisterModel, Attendance, AttendanceReport, LeaveReportStaff, FeedBackStaffs, StudentResult 
...
def staff_take_attendance(request): 
	subjects = Subjects.objects.filter(staff_id=request.user.id) 
	session_years = SemisterModel.objects.all() 
	context = { 
		"subjects": subjects, 
...
def get_students(request): 
	...
	session_model = SemisterModel.objects.get(id=session_year) 
...
def save_attendance_data(request): 
	...
	subject_model = Subjects.objects.get(id=subject_id) 
	session_year_model = SemisterModel.objects.get(id=session_year_id) 
...
def staff_profile_update(request): 
	if request.method == "POST":  
		...
		staff_data = {attr: request.POST.get(attr) for attr in attributes}

		try: 
			update_staff_profile(request.user, staff_data)
...
def update_staff_profile(user: CustomUser, staff_data: Dict[str, str]) -> None:
	customuser = CustomUser.objects.get(id=user.id) 
	customuser.first_name = staff_data.get('first_name')
	customuser.last_name = staff_data.get('last_name') 
	if password: 
		customuser.set_password(staff_data.get('password') ) 
	customuser.save() 

	staff = Staffs.objects.get(admin=customuser.id) 
	staff.address = staff_data.get('address') 
	staff.save()
...
def staff_add_result_save(request): 
	if request.method == "POST":  
		...
		try: 
			result = add_student_result(student_obj, subject_obj)
			result.save()
		except Exception: 
...
def add_student_result(student: Students, subject: Subjects) -> StudentResult:
	if result := StudentResult.objects.filter(subject_id=subject, student_id=student).first():
		result.subject_assignment_marks = assignment_marks
		result.subject_exam_marks = exam_marks
		messages.success(request, "Result Updated Successfully!")
	else:
		result = StudentResult(
			student_id=student_obj, subject_exam_marks=exam_marks,
